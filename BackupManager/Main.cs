...
namespace BackupManager
{
    ...
    public partial class Main : Form
    {
            ...
            string driveName)
        {
            ...
            {
                Utils.LogWithPushBullet(pushbulletApiKey, logFile, BackupAction.CheckBackupDisk, "ERROR: {0} has zerobyte hashcode", fullPath);
            }
            ...
            {
                Utils.LogWithPushBullet(pushbulletApiKey, logFile, BackupAction.CheckBackupDisk, "ERROR: {0} has incorrect hashcode", fullPath);
        ...
        private void CheckConnectedDisk(bool deleteExtraFiles)
        {
            ...
            {
                Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.CheckBackupDisk, "No connected backup disk detected.");
                return;
            }
            ...
            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.CheckBackupDisk, "Checking {0} - {1}GB free", backupFolderName, Utils.GetDiskFreeSpace(folderToCheck));
            ...
            foreach (string backupFileFullPath in backupDiskFiles)
            {
                ...
                if (this.mediaBackup.Contains(backupFileHash, backupFileFullPath))
                {
                    ...
                    if (!backupDiskFilename.Equals(masterFilename))
                    {
                        ...
                        Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.CheckBackupDisk, "Renaming {0} to {1}", backupFileFullPath, destinationFileName);
            ...
            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.CheckBackupDisk, "Completed.");
        }
        ...
        private void DeleteEmptyDirectories(string logFile, string startLocation)
        {
            foreach (var directory in Directory.GetDirectories(startLocation))
            {
                ...
                {
                    Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.CheckBackupDisk, "Deleting empty folder {0}", directory);
        ...
        private void CopyFiles()
        {
            ...
            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.BackupFiles, "Started.");
            ...
            foreach (BackupFile backupFile in filesToBackup)
            {
                    ...
                    else
                    {
                        ...
                        if (availableSpace > MinimumFreeSpaceToLeaveOnBackupDrive)
                        {
                            if (availableSpace > sourceFileInfo.Length)
                            {
                                ...
                                Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.BackupFiles, "Copying {0}", sourceFileName);
                                Directory.CreateDirectory(Path.GetDirectoryName(destinationFileName));
                            ...
                            else
                            {
                                ...
                                {
                                    Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.BackupFiles, "Skipping {0} as not enough free space", sourceFileName);
                ...
                catch (IOException)
                {
                    ...
                    Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.BackupFiles, "IOException during copy. Skipping file.");
                }
            }
            ...
            {
                Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "{0:n0} files still to backup", filesNotOnBackupDisk.Count());
            }
            ...
            {
                Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "{0:n0} files still without DiskChecked set", filesWithoutDiskChecked.Count());
            }

            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.BackupFiles, "Completed.");
        }
        ...
        private void ScanFolders()
        {
            ...
            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "Started.");

            foreach (string masterFolder in this.mediaBackup.MasterFolders)
            {
                Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "Scanning {0}", masterFolder);
                ...
                foreach (string indexFolder in this.mediaBackup.IndexFolders)
                {
                    ...
                    {
                        Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "Scanning {0}", folderToCheck);
            ...
            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "{0:n0} files at {1:n0}MB", totalFiles, b / 1024 / 1024);

            if (oldestFile != null)
            {
                Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "Oldest backup date is {0:n0} days ago at {1} for {2} on {3}", DateTime.Today.Subtract(oldestFileDate).Days, oldestFileDate.ToShortDateString(), oldestFile.GetFileName(), oldestFile.BackupDisk);
            }

            IEnumerable<BackupFile> filesNotOnBackupDisk =
               this.mediaBackup.BackupFiles.Where(p => string.IsNullOrEmpty(p.BackupDisk));

            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "{0:n0} files to backup", filesNotOnBackupDisk.Count());

            IEnumerable<BackupFile> filesWithoutDiskChecked =
               this.mediaBackup.BackupFiles.Where(p => string.IsNullOrEmpty(p.BackupDiskChecked));

            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "{0:n0} files without DiskChecked set", filesWithoutDiskChecked.Count());

            Utils.LogWithPushBullet(this.mediaBackup.PushBulletApiKey, logFile, BackupAction.ScanFolders, "Completed.");
        }
