...
internal sealed partial class Main
{
    ...
    private void ScanAllDirectories(bool updateLastFullScan, CancellationToken ct)
    {
        ...
        foreach (var backupFile in backupFilesWithoutDeleted)
        {
            if (!fileBlockingCollection.Contains(backupFile.FullPath)) Utils.Log($"{backupFile.FullPath} is not in the blockingCollection");
            backupFilesFileNameHashSet.Add(backupFile.FullPath);
        }
        var fileNames = fileBlockingCollection.ToArray();

        foreach (var fileName in fileNames.Where(fileName => !backupFilesFileNameHashSet.Contains(fileName)))
        {
